...
import { ApplicationCommandOption, Client, CommandInteraction, PermissionString, VoiceChannel } from 'discord.js';
 ...
 */
const description = 'Only move members that have a specific role.';
...
async function execute(client: Client, interaction: CommandInteraction) {
    ...
    const from = interaction.options.get('from')?.channel as VoiceChannel;
    ...
    // Filter members
    const members = Array.from(channel1.members.values()).filter(m => m.roles.cache.has(role.id));
